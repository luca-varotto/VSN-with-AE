...
plotting_tools = SourceFileLoader('somelib', join(PROJECT_PATH, 'plotting_tools.py')).load_module()
...
class GMM:
  ...
  """
  def GMM_generation(self, X, M_range, type = 'standard',weight_concentration_prior_type='dirichlet_distribution', weight_concentration_prior=1.0E-3,want_to_plot=False):
    if type == 'standard': 
      # choose the best number of components for the GMM according to the BIC criterion
      models = [GaussianMixture(n, covariance_type='full', init_params='kmeans',max_iter=300,n_init=1).fit(X) for n in M_range] # fit through EM algotithm
      ...
      gmm = BayesianGaussianMixture(n_components=M_range[-1],covariance_type='full', init_params='kmeans',weight_concentration_prior_type=weight_concentration_prior_type,weight_concentration_prior=weight_concentration_prior, max_iter=300,n_init=1).fit(X)
  ...
  def prediction(self,X,probs=[], labels= [],want_to_plot=False):
    ...
    if want_to_plot and num_features == 2:
      ...
      plotting_tools.plot_gmm(self.gmm_model.means_, self.gmm_model.covariances_, self.gmm_model.weights_, X, scale, labels)
...
class kMeans:
  ...
  def prediction(self,X,want_to_plot=False):
    ...
    num_features = np.shape(X)[1]
    if want_to_plot and num_features == 2:
      plotting_tools.plot_kmeans(self.kmeans_model.cluster_centers_, X, labels)
